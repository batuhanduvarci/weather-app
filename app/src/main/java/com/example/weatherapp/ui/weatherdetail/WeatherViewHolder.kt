...
class WeatherViewHolder(private val view : View) : RecyclerView.ViewHolder(view) {
    fun bind(weather: DailyWeatherItemModel) {
        ...
        with(binding){
            ...
            weather.temp?.let {
                degreeHighestTextView.text = String.format(view.resources.getString(R.string.fragment_weather_detail_degree_text), it.max)
                degreeLowestTextView.text = String.format(view.resources.getString(R.string.fragment_weather_detail_degree_text), it.min)
            }
        }
    }
}